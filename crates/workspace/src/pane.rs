...
impl Pane {
    ...
    fn tab_icon_element(
        ...
        let Some(diagnostic) = item_diagnostic else {
            return item.tab_icon_element(window, cx).or_else(|| {
                tab_icon.map(|icon| {
        ...
        let icon = tab_icon?.size(IconSize::Small).color(Color::Muted);
                ...
                .menu(move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _, _| {
                        menu.action("New File", NewFile.boxed_clone())
                            ...
                            .separator()
                            .action("Search Workspace", DeploySearch::default().boxed_clone())
